...
namespace Tests.Model3;

public class Model3PredictorTests
{
    private readonly Model3Predictor _model3Predictor = new ();
    
    [Fact]
    public void Generate_GeneratesCorrectYearValues()
    {
        var inputs = new Model3Inputs
        {
            StartYear = 2020,
            AgeAtStart = 30,
            EndYear = 2023,
        ...
        // act
        var prediction = _model3Predictor.Generate(Substitute.For<ReturnRateCalculator>(), inputs);
    ...
    public void Generate_WithSingle_Allocation_ShouldCalculateInvestmentReturn()
    {
        ...
        // act
        var prediction = _model3Predictor.Generate(returnRateCalculator, inputs);
    ...
    public void Generate_WithTwo_Allocations_ShouldCalculateInvestmentReturn()
    {
        ...
        var returnRateCalculator = Substitute.For<IReturnRateCalculator>();
        returnRateCalculator.GetReturnRate(0.05m, 0.25m).Returns(0.05m);
        returnRateCalculator.GetReturnRate(0.01m, 0).Returns(0.01m);
